...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyA.categoryBitMask & itemCategory) == itemCategory || (contact.bodyB.categoryBitMask & itemCategory) == itemCategory {
            ...
            let music = SKAction.playSoundFileNamed("coin.mp3", waitForCompletion: true)
            self.run(_:music)
    ...
    func setupitem() {
        ...
        let createitemAnimation = SKAction.run({
            ...
            let item_random_y_range = self.frame.size.height / 4
            
            // 1〜random_y_rangeまでのランダムな整数を生成
            let item_random_y = arc4random_uniform( UInt32(item_random_y_range) )
            // Y軸の下限にランダムな値を足して、アイテムのY座標を決定
            let under_item_y = CGFloat(300 + item_random_y)
            
            // アイテムを作成
            let itemCreat = SKSpriteNode(texture: itemTexture)
            itemCreat.position = CGPoint(x: 100.0, y: under_item_y)
            
          
            // 物理演算を設定
            itemCreat.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
            itemCreat.physicsBody?.categoryBitMask = self.itemCategory
            
            // 衝突の時に動かないように設定する
            itemCreat.physicsBody?.isDynamic = false
            

            
            item.addChild(itemCreat)
